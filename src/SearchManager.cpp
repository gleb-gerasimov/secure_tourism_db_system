...
SearchManager::SearchManager(Database *database) : m_database(database) {}

QVector<Record> SearchManager::findTours(const std::optional<QDate> &startDate, const std::optional<QDate> &endDate,
    ...
    for (const Record &record: records) {
        ...
        if (startDate.has_value() && record.startDate < startDate.value()) {

            matches = false;
        }

        if (endDate.has_value() && record.endDate > endDate.value()) {

            matches = false;
        }

        if (country.has_value() && record.country != country.value()) {
        ...
        if (hasFlight.has_value() && record.hasFlight != hasFlight.value()) {

            matches = false;
        }

        if (participants.has_value() && record.participants != participants.value()) {

            matches = false;
        }

        if (price.has_value() && record.price != price.value()) {
